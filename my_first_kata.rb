...
shop_items_by_index_original.sort!.reverse!
...
(2..5).reverse_each {|v| 
	...
	if shop_items_by_index[v-1]!=0 
		...
		puts "#{(shop_items_by_index==[0,0,0,0,0])}"
				...
				if (shop_items_by_index!=[0,0,0,0,0]) 
					...
					while (shop_items_by_index[x-1]>0) 
						puts "shop_items_by_index[x-1] #{(shop_items_by_index[x-1]>0)}"
		...
		puts "-------array lote    #{lots_array } +++++ #{shop_items_by_index[0]}"
		mega_lots_array[5-v]=lots_array

		puts "mega_lots_array  #{mega_lots_array}"
		price_array[v-2]=8*(lots_array[0]*0.75*5+lots_array[1]*0.80*4+lots_array[2]*0.90*3+lots_array[3]*0.95*2+shop_items_by_index[0])
